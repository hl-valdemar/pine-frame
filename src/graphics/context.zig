...
// //
// //         const config = c.PineSwapchainConfig{
...
// //     swapchain.context.backend.present.?(swapchain.handle);
// // }
//
// const std = @import("std");
// const builtin = @import("builtin");
...
pub fn beginFrame(context: *Context) void {
    if (context.backend.begin_frame) |begin_frame| {
        begin_frame();
    }
}

pub fn endFrame(context: *Context) void {
    if (context.backend.end_frame) |end_frame| {
        end_frame();
    }
}
